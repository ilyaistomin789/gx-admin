...
} & Pick<ReturnType<typeof useFormSF>, 'defaultFormValuesLoading'>;
...
> => {
  ...
  const formSF = useFormSF({
    form: formAnt,
    defaultFormValues: defaultFormValues as
      | Store
      | (() => Promise<Store> | Store),
  });
  const { form } = formSF;
  ...
  return {
    ...
    formProps: {
      ...
      onKeyUp,
      onValuesChange,
      initialValues: (toEditModelConverter && query?.data?.data
        ? toEditModelConverter(query?.data?.data)
        : query?.data?.data) as Store | undefined,
    },
    saveButtonProps,
